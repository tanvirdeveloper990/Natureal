...
<div class="container py-5">
    ...
    <div class="d-flex justify-content-between align-items-center border-bottom pb-3 mb-4">
        <h2 class="fw-semibold text-dark mb-0">Vendor Order Report</h2>
    ...
    <form method="GET" class="no-print mb-4 bg-white p-4 rounded shadow-sm">
                ...
                <select name="vendor_id" class="form-select">
                    ...
                    @foreach($vendors as $vendor)
                        <option value="{{ $vendor->id }}" 
                            @if(request()->get('vendor_id') == $vendor->id) selected @endif>
                            {{ $vendor->name }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="mt-4 d-flex gap-2">
            ...
            <a href="{{ route('vendor.vendor_order_report') }}" class="btn btn-danger">Reset</a>
        </div>
    </form>

    <!-- Pagination -->
    <div class="d-flex justify-content-between align-items-center mb-3 no-print">
        <div>{{ $orders->onEachSide(1)->links('vendor.pagination.custom-tailwind') }}</div>
    </div>
